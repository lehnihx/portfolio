  ...
  return (
    <motion.div {...fade(0.2)}>
      <div className="flex items-center justify-between mb-6">
        <p className="text-[11px] tracking-[3px] text-foreground/40 uppercase">Used Languages</p>
        ...
        {filtered.map((lang, index) => {
          ...
          return (
            <div key={lang.name} className="flex items-center gap-4">
              <p className="w-14 text-right text-[12px] text-foreground/60 shrink-0">{lang.name}</p>
              ...
              <p className="w-8 text-[11px] text-foreground/30 tabular-nums">{pct.toFixed(1)}%</p>
            </div>
          )
        })}
      </div>
